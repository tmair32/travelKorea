...
var stamps = d3.select("#stamplist")
console.log(stamps)
...
d3.json('/static/accounts/assets/data/plot/kor_admin_1.topojson').then(function(data1){
    var stamps = document.getElementById('stamplist').options;
    console.log(stamps)
    ...
    function redraw(selected) {
        ...
        area.enter().append("path")
            ...
            .attr("fill", function(s, i) {
                return (s.properties.LVL_1_EN==null)?"#000000":color(1);
            })
            .style("fill-opacity", defOpacity);
        
        for (i=1; i < stamps.length; i++) {
            console.log(stamps[i])
            console.log('mapx', stamps[i].value)
            console.log('mapy', stamps[i].text)

            var mapxy = [stamps[i].value, stamps[i].text]
    ...
    d3.json('/static/accounts/assets/data/plot/kor_admin_2.topojson').then(function(data2){
        // Read level 3 map data
        d3.json('/static/accounts/assets/data/plot/kor_admin_3.topojson').then(function(data3){
